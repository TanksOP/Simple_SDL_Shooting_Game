	...
	void update() {
		for (auto &m : meteors) {
			...
			for (auto& b : bulletManager->bullets) {
				SDL_Rect bulletRect = { b.x, b.y, 10, 10 }; // collision box of bullet
				SDL_Rect meteorRect = { m.x, m.y, 32, 32 }; // collition box of meteor
				SDL_Rect nullRect;
		...
		auto remove = std::remove_if(meteors.begin(), meteors.end(),
			[](const Meteor& m) {return m.x == 0xCCCCCCCC; });
